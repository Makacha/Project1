...
class Queen(Line, Diagonal, CanPin):

    def __init__(self, pos: (int, int), color):
        self.color = color
        img_path = settings.PIECES_IMAGE_PATH + color + "-queen.png"
        self.image = pygame.image.load(img_path)
        super().__init__(pos)

    def able_capture(self, x: int, y: int, board, old=None, new=None):
        return self.able_capture_line(x, y, board, old, new) or self.able_capture_diagonal(x, y, board, old, new)
